...
// Packed-nibble weight tile for the IU4 path: 8 weights per int instead of 4,
// so the quantized region halves (32 ints vs 64). Same 8 scales + 4 pad; 44 % 8
// == 4 keeps the bank-conflict padding rule.
#define MMQ_MMA_TILE_X_K_ROCMI4 (  MMQ_TILE_NE_K + 2*MMQ_TILE_NE_K/QI8_0                   + 4)
...
template <int mmq_y, bool need_check> static __device__ __forceinline__ void load_tiles_rocmfp4(
    const char * __restrict__ x, int * __restrict__ x_tile, const int kbx0, const int i_max, const int stride) {
    constexpr int nwarps = mmq_get_nwarps_device();
    constexpr int warp_size = ggml_cuda_get_physical_warp_size();
    ...
    float * x_df = (float *) (x_qs + MMQ_TILE_NE_K*2);
#else
    constexpr tile_x_sizes txs = mmq_get_dp4a_tile_x_sizes(GGML_TYPE_Q4_0_ROCMFP4, mmq_y);
    int   * x_qs = (int   *)  x_tile;
    float * x_df = (float *) (x_qs + txs.qs);
#endif // defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE) || defined(AMD_WMMA_AVAILABLE)
...
#pragma unroll
...
#if defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE) || defined(AMD_WMMA_AVAILABLE)
        x_qs[i*MMQ_MMA_TILE_X_K_Q3_K + k0 + 0]          = v.x;
        x_qs[i*MMQ_MMA_TILE_X_K_Q3_K + k0 + QI_ROCMFP4] = v.y;
#else
        x_qs[i*(2*MMQ_TILE_NE_K + 1) + k0 + 0]          = v.x;
    ...
    constexpr int blocks_per_tile_x_row = MMQ_TILE_NE_K / QI_ROCMFP4;
    constexpr int rows_per_warp = warp_size / blocks_per_tile_x_row;
    const int kbxd = threadIdx.x % blocks_per_tile_x_row;
...
#pragma unroll
    for (int i0 = 0; i0 < mmq_y; i0 += nwarps * rows_per_warp) {
        ...
        const block_rocmfp4 * bxi = (const block_rocmfp4 *) x + kbx0 + i*stride + kbxd;

#if defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE) || defined(AMD_WMMA_AVAILABLE)
        x_df[i*MMQ_MMA_TILE_X_K_Q3_K + 2*kbxd + 0] = rocmfp4_ue4m3_to_fp32_half_finite(bxi->e[0]);
        x_df[i*MMQ_MMA_TILE_X_K_Q3_K + 2*kbxd + 1] = rocmfp4_ue4m3_to_fp32_half_finite(bxi->e[1]);
#else
        x_df[i*(2*MMQ_TILE_NE_K*2/QI8_0) + i/(QI8_0/4) + 2*kbxd + 0] = rocmfp4_ue4m3_to_fp32_half_finite(bxi->e[0]);
        x_df[i*(2*MMQ_TILE_NE_K*2/QI8_0) + i/(QI8_0/4) + 2*kbxd + 1] = rocmfp4_ue4m3_to_fp32_half_finite(bxi->e[1]);
#endif // defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE) || defined(AMD_WMMA_AVAILABLE)
    }
}
...
#pragma unroll
    for (int i0 = 0; i0 < mmq_y; i0 += nwarps * rows_per_warp) {
        ...
        const float d = rocmfp4_ue4m3_to_fp32_half_finite(bxi->e);
...
template <int mmq_y, bool need_check> static __device__ __forceinline__ void load_tiles_rocmi4(
    const char * __restrict__ x, int * __restrict__ x_tile, const int kbx0, const int i_max, const int stride) {
    constexpr int nwarps = mmq_get_nwarps_device();
    constexpr int warp_size = ggml_cuda_get_physical_warp_size();
    ...
    float * x_df = (float *) (x_qs + MMQ_TILE_NE_K*2);
#else
    constexpr tile_x_sizes txs = mmq_get_dp4a_tile_x_sizes(GGML_TYPE_Q4_0_ROCMI4, mmq_y);
    int   * x_qs = (int   *)  x_tile;
    float * x_df = (float *) (x_qs + txs.qs);
#endif // defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE) || defined(AMD_WMMA_AVAILABLE)
...
//
// The dp4a/int8-WMMA loader above sign-extends every nibble to int8, which
// costs 2x the LDS and throws away the fact that ROCMI4 codes already ARE
// two's-complement [-8,+7] values -- i.e. exactly the operand format that
// v_wmma_i32_16x16x16_iu4 consumes. This loader keeps them packed.
//
// On disk a block is the usual split-half layout: qs[j] low nibble = element j,
// high nibble = element j+16. WMMA needs 8 *consecutive* K elements per dword,
// so the loader repacks. Cost is paid once per tile and amortised over all
// mmq_x columns, so it is free in practice.
// ---------------------------------------------------------------------------

// K-order note: a dot product is invariant under any permutation of K applied
// identically to both operands. Rather than compacting nibbles into consecutive
// element order (~22 ALU ops per dword), both the weight and activation packers
// below emit the byte-interleaved order
//     [b+0, b+4, b+1, b+5, b+2, b+6, b+3, b+7]
// which falls out of a single mask-and-shift-by-4 merge (~5 ops). As long as
// both sides agree, the WMMA result is identical.

// Low nibbles of two adjacent on-disk dwords, byte-interleaved.
static __device__ __forceinline__ int rocmi4_pack_lo(const int a, const int b) {
    return (a & 0x0F0F0F0F) | ((b & 0x0F0F0F0F) << 4);
}

// Same for the high nibbles (elements +16).
...
template <int mmq_y, bool need_check> static __device__ __forceinline__ void load_tiles_rocmi4_w4a4(
    ...
    // 2 threads per 32-element block, 8 blocks per MMQ_ITER_K=256 row.
...
#pragma unroll
    for (int i0 = 0; i0 < mmq_y; i0 += nrows*nwarps) {
        ...
        const int d0 = rocmfp4_get_qs_i32(bxi->qs, 2*half + 0);
...
template <int mmq_y, bool need_check> static __device__ __forceinline__ void load_tiles_rocmfpx_fp2(
    ...
    float * x_df = (float *) (x_qs + MMQ_TILE_NE_K*2);
#else
    constexpr tile_x_sizes txs = mmq_get_dp4a_tile_x_sizes(GGML_TYPE_Q2_0_ROCMFPX, mmq_y);
    int   * x_qs = (int   *)  x_tile;
    float * x_df = (float *) (x_qs + txs.qs);
#endif
    ...
    constexpr int nrows = warp_size / threads_per_row;
    const int txi = warp_size > threads_per_row ? threadIdx.x % threads_per_row : threadIdx.x;
    const int kbx = txi / QI_ROCMFP2;
    const int kqsx = txi % QI_ROCMFP2;
...
#pragma unroll
    for (int i0 = 0; i0 < mmq_y; i0 += nrows*nwarps) {
        ...
        const int v0 = rocmfpx_pack4_fp2_vec_cuda(bxi[0].qs[kqsx]);
        const int v1 = rocmfpx_pack4_fp2_vec_cuda(bxi[MMQ_TILE_NE_K/QI_ROCMFP2].qs[kqsx]);
#if defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE) || defined(AMD_WMMA_AVAILABLE)
        x_qs[i*MMQ_MMA_TILE_X_K_Q3_K + 0             + txi] = v0;
        x_qs[i*MMQ_MMA_TILE_X_K_Q3_K + MMQ_TILE_NE_K + txi] = v1;
#else
        x_qs[i*(2*MMQ_TILE_NE_K + 1) + 0             + txi] = v0;
        x_qs[i*(2*MMQ_TILE_NE_K + 1) + MMQ_TILE_NE_K + txi] = v1;
#endif
    }
...
#else
        x_df[i*(2*MMQ_TILE_NE_K*2/QI8_0) + i/(QI8_0/4) + 2*kbxd + 0] = rocmfpx_ue4m3_to_fp32_finite(bxi->e[0]);
        x_df[i*(2*MMQ_TILE_NE_K*2/QI8_0) + i/(QI8_0/4) + 2*kbxd + 1] = rocmfpx_ue4m3_to_fp32_finite(bxi->e[1]);
#endif
    }
}

template <int mmq_y, bool need_check> static __device__ __forceinline__ void load_tiles_rocmfpx_fp3(
    const char * __restrict__ x, int * __restrict__ x_tile, const int kbx0, const int i_max, const int stride) {
    constexpr int nwarps = mmq_get_nwarps_device();
    constexpr int warp_size = ggml_cuda_get_physical_warp_size();
    ...
    float * x_df = (float *) (x_qs + MMQ_TILE_NE_K*2);
#else
    constexpr tile_x_sizes txs = mmq_get_dp4a_tile_x_sizes(GGML_TYPE_Q3_0_ROCMFPX, mmq_y);
    int   * x_qs = (int   *)  x_tile;
    float * x_df = (float *) (x_qs + txs.qs);
#endif // defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE) || defined(AMD_WMMA_AVAILABLE)

    // Same thread layout as Q8_0: 32 values per block, 8 blocks per 256-value tile row.
    constexpr int threads_per_row = 32;
    constexpr int nrows = warp_size / threads_per_row;
    const int txi = warp_size > threads_per_row ? threadIdx.x % threads_per_row : threadIdx.x;
    const int kbx  = txi / QI_ROCMFP3;
    const int kqsx = txi % QI_ROCMFP3;
...
template <int mmq_y, bool need_check> static __device__ __forceinline__ void load_tiles_rocmfpx_fp6(
    ...
    float * x_df = (float *) (x_qs + MMQ_TILE_NE_K*2);
#else
    constexpr tile_x_sizes txs = mmq_get_dp4a_tile_x_sizes(GGML_TYPE_Q6_0_ROCMFPX, mmq_y);
    int   * x_qs = (int   *)  x_tile;
    float * x_df = (float *) (x_qs + txs.qs);
#endif // defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE) || defined(AMD_WMMA_AVAILABLE)
    ...
    constexpr int nrows = warp_size / threads_per_row;
    const int txi = warp_size > threads_per_row ? threadIdx.x % threads_per_row : threadIdx.x;
    const int kbx  = txi / QI_ROCMFP6;
    const int kqsx = txi % QI_ROCMFP6;
...
template <int mmq_y, bool need_check> static __device__ __forceinline__ void load_tiles_rocmfpx_fp8(
    const char * __restrict__ x, int * __restrict__ x_tile, const int kbx0, const int i_max, const int stride) {
    constexpr int nwarps = mmq_get_nwarps_device();
    constexpr int warp_size = ggml_cuda_get_physical_warp_size();
    ...
    float * x_df = (float *) (x_qs + MMQ_TILE_NE_K*2);
#else
    constexpr tile_x_sizes txs = mmq_get_dp4a_tile_x_sizes(GGML_TYPE_Q8_0_ROCMFPX, mmq_y);
    int   * x_qs = (int   *)  x_tile;
    float * x_df = (float *) (x_qs + txs.qs);
#endif // defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE) || defined(AMD_WMMA_AVAILABLE)
    ...
    constexpr int nrows = warp_size / threads_per_row;
    const int txi = warp_size > threads_per_row ? threadIdx.x % threads_per_row : threadIdx.x;
    const int kbx  = txi / QI_ROCMFP8;
    const int kqsx = txi % QI_ROCMFP8;
...
#pragma unroll
    for (int i0 = 0; i0 < mmq_y; i0 += nrows*nwarps) {
        ...
        const block_rocmfp8 * bxi = (const block_rocmfp8 *) x + kbx0 + i*stride + kbx;

        const int qs0 = get_int_b2((const void *) bxi->qs, kqsx);
...
#pragma unroll
    for (int i0 = 0; i0 < mmq_y; i0 += nwarps * rows_per_warp) {
        ...
        const block_rocmfp8 * bxi = (const block_rocmfp8 *) x + kbx0 + i*stride + kbxd;

#if defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE) || defined(AMD_WMMA_AVAILABLE)
        x_df[i*MMQ_MMA_TILE_X_K_Q8_0                 + kbxd] = rocmfpx_ue4m3_to_fp32_finite(bxi->e);
#else
        x_df[i*(2*MMQ_TILE_NE_K/QI8_0) + i/(QI8_0/2) + kbxd] = rocmfpx_ue4m3_to_fp32_finite(bxi->e);
#endif // defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE) || defined(AMD_WMMA_AVAILABLE)
    }
}
...
#ifndef GGML_ROCMFP4_FAST_MMQ_CACHE_DA
#define GGML_ROCMFP4_FAST_MMQ_CACHE_DA 0
#endif

template <int mmq_x, int mmq_y, mmq_q8_1_ds_layout ds_layout>
static __device__ __forceinline__ void vec_dot_q8_0_q8_1_mma_cached_da(
    const int * __restrict__ x, const int * __restrict__ y, float * __restrict__ sum, const int k00) {
#if defined(AMD_MFMA_AVAILABLE) || defined(AMD_WMMA_AVAILABLE)
    constexpr data_layout input_layout = get_input_data_layout();
    typedef tile<16,  8, int, input_layout>        tile_A;
...
#pragma unroll
                for (int l = 0; l < tile_C::ne; ++l) {
                    sum[(j0/tile_C::J + n)*tile_C::ne + l] += C.x[l]*dA[n][l]*dB;
                }
            }
        }
    }
#else
    vec_dot_q8_0_q8_1_mma<mmq_x, mmq_y, ds_layout>(x, y, sum, k00);
#endif // defined(AMD_MFMA_AVAILABLE) || defined(AMD_WMMA_AVAILABLE)
}
...
template <int mmq_x, int mmq_y, bool need_check>
struct mmq_type_traits<mmq_x, mmq_y, need_check, GGML_TYPE_Q4_0_ROCMFP4> {
    static constexpr int              vdr          = VDR_ROCMFP4_Q8_1_MMQ;
    static constexpr load_tiles_mmq_t load_tiles   = load_tiles_rocmfp4<mmq_y, need_check>;
...
#if GGML_ROCMFP4_FAST_MMQ_CACHE_DA
    static constexpr vec_dot_mmq_t    vec_dot_mma  = vec_dot_q8_0_q8_1_mma_cached_da<mmq_x, mmq_y, MMQ_Q8_1_DS_LAYOUT_D4>;
#else
    static constexpr vec_dot_mmq_t    vec_dot_mma  = vec_dot_q8_0_q8_1_mma<mmq_x, mmq_y, MMQ_Q8_1_DS_LAYOUT_D4>;
#endif
    static constexpr vec_dot_mmq_t    vec_dot_dp4a = vec_dot_q8_0_q8_1_dp4a<mmq_x, mmq_y>;
...
template <int mmq_x, int mmq_y, bool need_check>
struct mmq_type_traits<mmq_x, mmq_y, need_check, GGML_TYPE_Q6_0_ROCMFPX> {
    static constexpr int              vdr          = VDR_ROCMFP6_Q8_1_MMQ;
    static constexpr load_tiles_mmq_t load_tiles   = load_tiles_rocmfpx_fp6<mmq_y, need_check>;
    static constexpr vec_dot_mmq_t    vec_dot_mma  = vec_dot_q8_0_16_q8_1_mma<mmq_x, mmq_y>;
    static constexpr vec_dot_mmq_t    vec_dot_dp4a = vec_dot_q8_0_16_q8_1_dp4a<mmq_x, mmq_y>;
};

template <int mmq_x, int mmq_y, bool need_check>
struct mmq_type_traits<mmq_x, mmq_y, need_check, GGML_TYPE_Q8_0_ROCMFPX> {
    static constexpr int              vdr          = VDR_ROCMFP8_Q8_1_MMQ;
    static constexpr load_tiles_mmq_t load_tiles   = load_tiles_rocmfpx_fp8<mmq_y, need_check>;
    static constexpr vec_dot_mmq_t    vec_dot_mma  = vec_dot_q8_0_q8_1_mma<mmq_x, mmq_y, MMQ_Q8_1_DS_LAYOUT_D4>;
    static constexpr vec_dot_mmq_t    vec_dot_dp4a = vec_dot_q8_0_q8_1_dp4a<mmq_x, mmq_y>;
};
...
#pragma unroll
        for (int n = 0; n < ntx; ++n) {
            load_ldmatrix(A[n], x_qs + (i0 + n*tile_A::I)*MMQ_MMA_TILE_X_K_ROCMI4 + kp, MMQ_MMA_TILE_X_K_ROCMI4);
        }
