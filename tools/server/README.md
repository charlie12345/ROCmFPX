...
| `--cache-disk PATH` | base directory for the automatic SSD-backed prompt cache (default: disabled); the server keeps an owner-only run directory below PATH and adopts entries left by previous runs<br/>(env: LLAMA_ARG_CACHE_DISK) |
