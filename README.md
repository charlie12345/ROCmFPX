...
`--tensor-read-lazy on` 会让大 PLE 张量使用 mmap 延迟读取，但 Windows 仍可能
把被访问的文件页留在进程工作集或系统缓存中。它不等于严格的“PLE 只放 SSD”。
...
该限制只对当前 `llama-server.exe` PID 生效，重启模型后需要重新执行。限制过低
可能增加 PLE 缺页读取并降低预填充速度；它也不会降低 GPU/UMA 提交量。
