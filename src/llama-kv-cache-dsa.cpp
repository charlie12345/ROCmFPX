...
llama_memory_context_ptr llama_kv_cache_dsa::init_batch(llama_batch_allocr & balloc, uint32_t n_ubatch, bool embd_all) {
...
bool llama_kv_cache_dsa::get_can_shift() const {
    return kv_mla->get_can_shift() && kv_lid->get_can_shift() && kv_mla->get_size() == kv_lid->get_size();
}
...
llama_kv_cache_dsa_context::llama_kv_cache_dsa_context(llama_kv_cache_dsa * kv) :
...
llama_kv_cache_dsa_context::llama_kv_cache_dsa_context(llama_kv_cache_dsa * kv, llama_context * lctx, bool optimize) :
