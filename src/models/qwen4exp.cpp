...
void llama_model_qwen4exp::load_arch_hparams(llama_model_loader & ml) {
    ...
    GGML_ASSERT(hparams.nextn_predict_layers < hparams.n_layer &&
                "nextn_predict_layers must be smaller than the full qwen4exp block count");
    ...
    // (bm3 keeps layer flags in recurrent_layer_arr; donor n_layer-all minus nextn = main layers)
    std::vector<uint32_t> recurrent_layers;
    if (ml.get_arr(LLM_KV_ATTENTION_RECURRENT_LAYERS, recurrent_layers, false) && !recurrent_layers.empty()) {
        for (uint32_t il : recurrent_layers) {
            GGML_ASSERT(il < hparams.n_layer);
    ...
    switch (hparams.n_layer - hparams.nextn_predict_layers) { // main layers only
...
void llama_model_qwen4exp::load_arch_tensors(llama_model_loader & ml) {
    ...
    // three nextn tensors. The target loader opens the complete target or draft GGUF, so
    // its appended decoder tensors are always materialized when present.
    for (int il = n_main; il < n_layer; ++il) {
        ...
        // The draft attends dense, so its indexer weights are never read.
...
llama_model_qwen4exp::graph_mtp::graph_mtp(const llama_model & model, const llm_graph_params & params)
    ...
    GGML_ASSERT(hparams.nextn_predict_layers == 1 && "qwen4exp MTP supports a single block");
...
ggml_tensor * llama_model_qwen4exp::graph::build_layer_attn(
    ...
    const bool qsa = mctx_hyb && mctx_hyb->get_idx() != nullptr && hparams.attn_compress_ratio[il] > 0;
