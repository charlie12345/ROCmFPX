...
// Mellum2 (JetBrains): MoE transformer with per-layer sliding-window attention.
//
// Key properties (Mellum2-12B-A2.5B):
//   - 28 layers, layer_types alternate sliding_attention x3 -> full_attention (period 4)
//   - QK-norm on every layer (RMS, per-head)
//   - MoE every layer (64 experts / 8 active, softmax gating, no shared expert)
//   - untied output head
//   - sliding layers use plain rope (no yarn), full layers use yarn-scaled rope
//
// Based on llama.cpp PR #23966 (Xarbirus).
