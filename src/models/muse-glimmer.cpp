...
llama_model_muse_glimmer::graph::graph(const llama_model & model, const llm_graph_params & params)
    ...
    // upstream passes an output-scale tensor here, but that tensor is created only for
    // GGML_TYPE_NVFP4 outputs; on the ROCmFP4 path it is always nullptr, and this
    // base's build_lora_mm takes no scale argument. Equivalent, not a maths change.
