...
// Based on:
//   https://github.com/Zyphra/vllm/blob/zaya1/vllm/model_executor/models/zaya.py
//   https://github.com/Zyphra/vllm/blob/zaya1/vllm/model_executor/layers/mamba/cca.py
//   llama.cpp draft PR #23112 (Juste-Leo2)

// Zaya-specific
// TODO: extend convolution op to support groups (like PyTorch Conv1d groups)
...
void llama_model_zaya::load_arch_tensors(llama_model_loader &) {
    ...
    // final residual scaling (after all layers)
    ...
    for (int i = 0; i < n_layer; ++i) {
        ...
        // input norm (shared by all layers)
        layer.attn_norm = create_tensor(tn(LLM_TENSOR_ATTN_NORM, "weight", i), {n_embd}, 0);

        // CCA attention layers (even indices)
        if (i % 2 == 0) {
            // Q, K projections
            ...
            // value projections (two time streams)
            ...
            // learned K temperature scaling (after L2 norm)
            layer.cca_k_scale = create_tensor(tn(LLM_TENSOR_CCA_K_SCALE, "weight", i), {n_head_kv}, 0);
        }

        // per-layer residual scaling: hidden_states = (hs + bias) * scale
...
llama_model_zaya::graph::graph(const llama_model & model, const llm_graph_params & params)
    ...
    // ResidualScaling: x = (x + bias) * scale
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        if (il % 2 == 0) {
            ...
            // recurrent state: conv_state | prev_hs
            ...
            // decode: from recurrent state; prefill: one-token shift
            ...
            // V = concat(val_proj1(cur), val_proj2(hs_d))
            ...
            // QKraw = concat(Qraw, Kraw)
            ggml_tensor * QKraw = ggml_concat(ctx0, Qraw, Kraw, 0);
            cb(QKraw, "QKraw", il);

            // qk_mean computation (grouped Q-K averaging)
            ...
            // grouped conv
            // cca_conv_grp is stored as BF16; cast to F16 for quantized models, BF16 for F32 models
            ...
            // add qk_mean back to Q, K
            Qcur = ggml_add(ctx0, Qcur, qk_mean_q);
            Kcur = ggml_add(ctx0, Kcur, qk_mean_k);

            // L2 normalization + learned K temperature scaling
            ...
            // RoPE (NeoX style, partial_rotary_factor=0.5)
            ...
            // gate_probs = first n_expert columns (exclude skip expert)
